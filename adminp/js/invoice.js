...
$(".addmore").on('click',function(){
 	html = '<tr>';
	html += '<td><input class="case" type="checkbox"/><input type="hidden" data-type="productCode" name="itemNo[]" id="itemNo_'+i+'" class="form-control autocomplete_txt" autocomplete="off"></td><td><input type="text" data-type="productName" name="itemName[]" id="itemName_'+i+'" class="form-control autocomplete_txt  validate[required] text-input" autocomplete="off"></td>';
	html += '<td><input type="text" name="price[]" id="price_'+i+'" class="form-control changesNo  validate[required] text-input" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" value="0.00" onpaste="return false;"></td>';
	html += '<td><input type="text" name="quantity[]" value="1" id="quantity_'+i+'" class="form-control changesNo  validate[required] text-input" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
	html += '<td align="right"><i class="fa fa-inr" aria-hidden="true"></i>&nbsp;<span id="bingos'+i+'" style="text-align:right;font-weight:bold">0.00</span><input type="hidden" name="total[]" id="total_'+i+'" class="form-control totalLinePrice" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" value="1.00" onpaste="return false;"></td>';
	html += '</tr>';
	$('table').append(html);
	i++;
});
...
$(document).on('change keyup blur','.changesNo',function(){
	...
	if( quantity!='' && price !='' ){
		 $('#total_'+id[1]).val( (parseFloat(price)*parseFloat(quantity)).toFixed(2) );	
		  $('#bingos'+id[1]).html( (parseFloat(price)*parseFloat(quantity)).toFixed(2) );	
		  }
	calculateTotal();
});
...
function calculateTotal(){
	...
	$('#total_payble').val( total.toFixed(2) );
	$('#balance_payble').val( total.toFixed(2) );
	calculateAmountDue();
}
