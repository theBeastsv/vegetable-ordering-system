<? 
	...
	$mafvshow= show_selecttable($debug,$limitstring,$conn,'size_color','*',"WHERE rowId=\"$pagegetdata\"",$limit,1,' rowId ASC');
	...
	foreach( $mafvshow as $row ) {} ?>
				...
				<div class="panel-heading"><i class="fa fa-external-link-square"></i>size_color<div class="panel-tools"></div>
								</div>
								<div class="panel-body">
			   <form class="form-horizontal" role="form" method="post" action="forms/edivariantsizecolor.php">
			   ...
			   <span><input type="hidden" placeholder="Companyid" class="form-control  " id="companyid" name="companyid" value="<?=$row['companyid'];?>"> </span> <div class="form-group">
											...
											<div class="col-sm-9">
												<input type="text" placeholder="Color_name" class="form-control  validate[required] text-input" id="color_name" name="color_name" value="<?=$row['color_name'];?>"> 
											</div>
											...
											<div class="col-sm-9">
												<input type="text" placeholder="Size_name" class="form-control  validate[required] text-input" id="size_name" name="size_name" value="<?=$row['size_name'];?>"> 
											</div>
										</div><div class="form-group">
											<label for="form-field-1" class="col-sm-2 control-label">
												Product_id
...
<div class="input-group">
		...
		foreach( $mafvshow as $ssrow ) {
			$tats=$ssrow['rowId'];
			if($tats==$row['product_masterid']){$simd="selected";}else{$simd="";}
 			echo '<option value="'.$tats.'" '.$simd .'>'.$tats.'</option>' ;
