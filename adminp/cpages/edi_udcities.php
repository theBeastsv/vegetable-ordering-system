<? 
	...
	$mafvshow= show_selecttable($debug,$limitstring,$conn,'ud_cities','*',"WHERE rowId=\"$pagegetdata\"",$limit,1,' rowId ASC');
	...
	foreach( $mafvshow as $row ) {} ?>
				...
				<div class="panel-heading"><i class="fa fa-external-link-square"></i>ud_cities<div class="panel-tools"></div>
								</div>
								<div class="panel-body">
			   <form class="form-horizontal" role="form" method="post" action="forms/ediudcities.php">
...
<div class="input-group">
		<select class="form-control  validate[required] text-input" id="state_id" name="state_id"><option value="">Select One</option> <?  
		  $mafvshow= show_selecttable($debug,$limitstring,$conn,'ud_states','rowId','',$limit,1,' rowId ASC');
		foreach( $mafvshow as $ssrow ) {
			$tats=$ssrow['rowId'];
			if($tats==$row['state_id']){$simd="selected";}else{$simd="";}
 			echo '<option value="'.$tats.'" '.$simd .'>'.$tats.'</option>' ;
