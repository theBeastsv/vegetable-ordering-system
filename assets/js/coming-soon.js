...
$(function() {
   ...
   $('.soon-menu-toggle').on('click', function(e) {
       $(this).toggleClass('open');       $('.soon-nav').slideToggle(400);       e.preventDefault();
   ...
   function removeHash () { 
	    ...
	    if ("pushState" in history)
	        ...
	        scrollH = document.body.scrollLeft;	        loc.hash = "";	        // Restore the scroll offset, should be flicker free
	        document.body.scrollTop = scrollV;
	        document.body.scrollLeft = scrollH;
	    }
	}   // Show related section when menu clicked
   var soonMenu =  $('.soon-menu'),
   	soonSections = $('.soon-sections');   soonMenu.find('a').on('click', function() {
   	var target = $(this).attr('href'),
   		targetEl = $(target),
   		waitDur = 700;		if ($(this).closest('li').hasClass('active')) {
			return;
		}		if (!targetEl.length) {
			return;
	...
	$('.close-sections-btn').on('click', function(e) {
		...
		soonSections.find('.active').removeClass('active');		removeHash();		$('.soon-menu-toggle').removeClass('open');
       $('.soon-nav').slideUp(400);		e.preventDefault();
	});
}); 